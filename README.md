...
cookiecutter gh:unknowncoder05/EducationEphemeralCookieCutter

# Or from a local directory
cookiecutter /path/to/EducationEphemeralCookieCutter
```
...
| `description` | A full-stack education platform for schools, courses, and cohorts | Project description |
...
## Abstract Capability Modules

This template keeps domain logic split into business-agnostic backend capability packages:

- `pm-content`: posts, comments, reactions, bookmarks, mentions, and feed primitives
- `pm-community`: profiles, follows, blocks, circles, memberships, and social graph primitives
- `pm-chat`: direct messages, conversations, thread metadata, and read-state primitives
- `pm-notifications`: inbox notifications, delivery preferences, and digests
- `pm-moderation`: reports, moderation queues, policy enforcement, and audit trails
- `pm-media`: image and video asset metadata, attachments, and storage references
- `pm-catalog`: hierarchical learning/content catalogs (tracks, units, sections, resources)
- `pm-cohorting`: time-bound participant groups and membership lifecycle
- `pm-enrollment`: access/entitlement lifecycle across participants and offerings
- `pm-assessment`: evaluations, attempts, submissions, and scoring workflows
- `pm-progress`: completion and mastery state tracking
- `pm-credentialing`: certificates, badges, and outcome artifacts
- `pm-scheduling`: session windows, attendance events, and time orchestration
- `pm-policy`: policy and rule evaluation hooks for configurable business behavior
- `pm-analytics`: event aggregation and performance signal foundations

All capability packages are enabled via environment flags and designed to remain policy-driven rather than hardcoded to any single business model.

The generated backend installs these modules from the `community-packages` GitHub repository, pinned to a specific commit for reproducible builds.
